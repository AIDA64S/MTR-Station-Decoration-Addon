...
public class CatenaryDataFileSaveModule extends CatenaryDataModuleBase {
    ...
    private final List<BlockPos> dirtyCatenaryPositions = new ArrayList<>();
    ...
    public CatenaryDataFileSaveModule(CatenaryData catenaryData, Level world, Map<BlockPos, Map<BlockPos, Catenary>> catenaries, Path savePath) {
        ...
        try {
            Files.createDirectories(catenariesPath);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public void load() {
        existingFiles.clear();
        readMessagePackFromFile(catenariesPath, CatenaryEntry::new, catenaryEntry -> catenaries.put(catenaryEntry.pos, catenaryEntry.connections), true);
    ...
    public void autoSave() {
        ...
        if (canAutoSave && checkFilesToDelete.isEmpty()) {
            ...
            filesWritten = 0;
            filesDeleted = 0;
            dirtyCatenaryPositions.addAll(catenaries.keySet());
    ...
    public boolean autoSaveTick() {
        if (canAutoSave) {
            ...
            boolean hasSpareTime = writeDirtyDataToFile(dirtyCatenaryPositions, pos -> catenaries.containsKey(pos) ? new CatenaryEntry(pos, catenaries.get(pos)) : null, BlockPos::asLong, catenariesPath);
            final boolean doneWriting = dirtyCatenaryPositions.isEmpty();
